...
fn visualize_filter(filter: &Filter, real: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (image_width, image_height) = (
        filter_to_make.num_columns() as u32,
        filter_to_make.num_rows() as u32,
    );
    let mut imgbuf: ImageBuffer<Luma<u8>, Vec<u8>> = ImageBuffer::new(image_height, image_width);

    let max_value = filter_to_make
        .as_rows()
        .iter()
        .chain(filter.filter_imag.as_rows().iter())
        .flatten()
        .cloned()
        .fold(f64::MIN, f64::max);
    let min_value = filter_to_make
        .as_rows()
        .iter()
        .chain(filter.filter_imag.as_rows().iter())
        .flatten()
